...
export default async function CountryDetailsPage({ params }) {
  ...
  return (
    <main className="mx-auto max-w-7xl px-5 py-10 lg:px-8">
      <Link
        className="mb-6 inline-flex text-sm font-semibold text-cyan-200 hover:text-white"
        href="/countries"
      >
        &lt;- Back to countries
      </Link>
      ...
      <section className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        ...
        <div className="grid gap-4">
          ...
          <InfoPanel title="Culture And Money">
            <Detail
              label="Languages"
              value={languages || "Not listed"}
            />
            <Detail
              label="Currencies"
              value={currencies || "Not listed"}
            />
            <Detail
              label="Time Zones"
              value={
                country.timezones?.join(", ") || "Not listed"
              }
            />
          </InfoPanel>
          ...
          <InfoPanel title="Map Coordinates">
            <Detail
              ...
              value={
                country.latlng?.join(", ") || "Not listed"
              }
            />
            <Detail
              label="Area"
              value={`${
                country.area?.toLocaleString() || "Unknown"
              } km²`}
            />
            <Detail
              label="Top-level Domain"
              value={country.tld?.join(", ") || "Not listed"}
            />
          </InfoPanel>
        </div>
      </section>
